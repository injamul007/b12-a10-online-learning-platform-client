      ...
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button
            ...
            className="lg:hidden p-2 rounded-md bg-white/5 hover:bg-white/6 transition"
          >
            <FiMenu />
          </button>
          ...
          {user ? (
            <>
              <div className="hidden sm:flex items-center gap-3">
                <img
                  src={user.photoURL || "https://i.ibb.co/bN9JNsZ/portfolio-course.jpg"}
                  alt={user.displayName || user.email}
                  className="w-9 h-9 rounded-full border-2 border-white/10"
                />
                <div className="text-sm">
                  <div className="font-medium text-white/90">{user.displayName || user.email.split("@")[0]}</div>
